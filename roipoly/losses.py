...
class MaskRasterizationLoss(nn.Module):
    ...
    def forward(self, preds, targets, target_len, lid=0):
        ...
        for i in range(targets.shape[0]):  # num_gt_boxes_batch
            ...
            tgt_poly_single = targets[i, :target_len[i]].view(-1, 2).unsqueeze(0)  # torch.Size([1, num_gt_corners_per_poly, 2])
            pred_poly_single = preds[i, :target_len[i]].view(-1, 2).unsqueeze(0)  # torch.Size([1, num_gt_corners_per_poly, 2])

            tgt_mask = self.gt_rasterizer(tgt_poly_single * float(resolution[1].item()), resolution[1].item(), resolution[0].item(), 1.0)  # torch.Size([1, 64, 64])
            tgt_mask = (tgt_mask + 1)/2

            pred_mask = self.pred_rasterizer(pred_poly_single * float(resolution[1].item()), resolution[1].item(), resolution[0].item(), 1.0)  # torch.Size([1, 64, 64])
